  ...
  if(ENa) {
    if(Me.r > 0.0) {
      ...
      if(Fd>0.0) {
        Fz=gl_FragCoord.z/gl_FragCoord.w;
        Ff=clamp(exp2(-Fd*Fd*Fz*Fz*1.442695),0.0,1.0);
        gl_FragColor=mix(vec4(0.0,0.0,0.0,1.0),gl_FragColor,Ff);
      }
    }
  }
  ...
  if(ENd) {
   if(Me.r > 0.0) {
     /* gl_FragColor=texture2D(Sd,Vu);*/
      ...
      Dw1=(Lc*Li* max(dot(Nm1,Ld),0.0) );
      Dw2=(Lc*Li* max(dot(Nm2,Ld),0.0) );
      Dw3=(Lc*Li* max(dot(Nm3,Ld),0.0) );
